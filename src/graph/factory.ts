...
export async function buildGraph(dbPath: string = 'file:preferences.db') {
  const llmClient = new OpenRouterService(config);
  const memoryService = await createMemoryService();
  const preferencesService = new PreferencesService(dbPath) // TODO: Implement preferences service and pass it to graph
